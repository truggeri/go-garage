...
Cookie-authenticated `POST`, `PUT`, and `DELETE` requests to `/api/v1` must
include a valid `X-CSRF-Token` header. Bearer-authenticated requests skip this
additional check. Safe methods remain exempt.

The token is generated by the existing CSRF middleware and exposed to
authenticated layouts as:
...
The shared htmx module listens for `showFlash`, creates the same flash markup
used by `web/templates/partials/flash-messages.html`, appends it to the
`.flash-messages` region, and ensures that region has `role="status"` and
