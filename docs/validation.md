...
| `service_type` | Must be valid enum value | "invalid service type" |
...
The `service_type` field is a fixed enumeration. Valid values are:
...
When `service_type` is set to `other`, a `custom_service_type` text field must be provided to describe the service.
